...
class CurlMulti extends AbstractHasDispatcher implements CurlMultiInterface
{
    ...
    public function remove(RequestInterface $request)
    {
        ...
        if (
            ($index = array_search($request, $this->requests, true)) !== false
        ) {
    ...
    {
        $multiException = new MultiTransferException(
            'Errors during multi transfer'
        );

        while ($e = array_shift($exceptions)) {
            $multiException->addFailedRequestWithException(
                $e['request'],
                $e['exception']
            );
        }
    ...
    private function executeHandles()
    {
        ...
        do {
            ...
            if (
                $active &&
                curl_multi_select($this->multiHandle, $selectTimeout) === -1
            ) {
     ...
     */
    protected function removeErroredRequest(
        RequestInterface $request,
        \Exception $e = null
    ) {
        $this->exceptions[] = ['request' => $request, 'exception' => $e];
        $this->remove($request);
        $this->dispatch(self::MULTI_EXCEPTION, [
            'exception' => $e,
    ...
    protected function processResponse(
        ...
        if (!$curlException) {
            if ($this->validateResponseWasSet($request)) {
                $state = $request->setState(RequestInterface::STATE_COMPLETE, [
                    'handle' => $handle,
                ]);
        ...
        $state = $request->setState(RequestInterface::STATE_ERROR, [
